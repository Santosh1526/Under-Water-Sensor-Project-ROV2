...
def open_graph_window():
    """Open a new window with real-time graphs of sensor data"""
    global graph_window
    ...
    graph_window.title("📊 Sensor Data Graphs")
    graph_window.geometry("1200x900")
    graph_window.configure(bg="#001F33")
    
    # Title
    title = tk.Label(graph_window, text="📊 Real-Time Sensor Data Visualization",
                    font=("Times", 20, "bold"), fg="#00E1FF", bg="#001F33")
    title.pack(pady=10)
    
    # Info label
    info_label = tk.Label(graph_window, 
                         text=f"📈 Displaying {len(sensor_data_list)} readings | Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
    ...
    refresh_btn = tk.Button(button_frame, text="🔄 Refresh Graphs", command=refresh_graphs,
    ...
    def export_graph():
        file_path = filedialog.asksaveasfilename(
            ...
            initialfile=f"sensor_graphs_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        )
        if file_path:
            try:
    ...
    # Close button
    close_btn = tk.Button(button_frame, text="❌ Close", command=graph_window.destroy,
...
download_sd_button = tk.Button(root, text="📥 Download SD", command=download_sd_card,
                               font=("Times", 11, "bold"), bg="#007A00", fg="white",
                               relief="flat", width=13, height=2, state="disabled")
download_sd_button.place(x=690, y=90)

graph_button = tk.Button(root, text="📊 View Graphs", command=open_graph_window,
                        font=("Times", 11, "bold"), bg="#FF8C00", fg="white",
                        relief="flat", width=13, height=2, state="disabled")
